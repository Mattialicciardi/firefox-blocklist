...
- `Sources/FirefoxBlocklist/main.swift`: unico file sorgente.
  ...
  - `SiteStore.apply()`: genera `policies.json` (solo i domini `enabled`) e lo
    scrive DIRETTAMENTE via syscall POSIX (`mkdir`/`open`/`write`/`rename`), come
    utente, senza `osascript`/admin, in `Firefox.app/Contents/Resources/distribution/`
    (unica posizione letta da Firefox su macOS; richiede il permesso "Gestione app",
    che si aggancia solo se il bundle è ri-firmato — vedi `scripts/install.sh`)
